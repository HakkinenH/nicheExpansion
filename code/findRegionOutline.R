...
#can use 4 var dataset instead, but will need to adjust later code
#clim123_ref <- na.exclude(read.csv("RawData/WorldClim/bioclim_4Var_10min_region.csv"))


clim123<-clim123_ref[,c("x","y", "bio5", "bio6", "bio12")]
...
niche_cal<-function(sp_name){
  ...
  writeRaster(z1$zz, paste0("IntermediateOutput/findRegionOutline/3Var/",sp_name,"PCA.tif"),overwrite=T)
  ...
  plotname=paste('IntermediateOutput/findRegionOutline/3Var/',sp_name,'_similarity.pdf', sep='')
