rm(list=ls())
setwd("U:/Data")
setwd("/Users/henry_hakkinen/Documents/ExeterWork/Data")
...
#source("species lists/niche_dynamic_functions_Nov11th2016HH.R")
...
niche_cal<-function(x){
  ...
  #file_natur=paste('species lists/desaggregated_data/',sp_name,'_naturalised.csv', sep='')
  #file_native=paste('species lists/desaggregated_data/',sp_name,'_native.csv', sep='')
  ...
  if (nrow(native)<=5 | nrow(natur)<=5){
    ...
    if (nrow(occ.sp1)<5 | nrow(occ.sp2)<5){
      ...
      if(a>0 & b>0){
        ...
        plot(shape,main=paste("Naturalised distribution",q))
        ...
        plotname=paste('niche_shift_work/analogue_disagg_21112018/Figures/',sp_name,q,'_PCA.png', sep='')
      ...
      write.csv(native_analogue100,paste('niche_shift_work/analogue_disagg_21112018/',q,'/',sp_name,q,'_native.csv', sep=''), row.names = FALSE)
      write.csv(naturalised_analogue100,paste('niche_shift_work/analogue_disagg_21112018/',q,'/',sp_name,q,'_natur.csv', sep=''), row.names = FALSE)
...
null_check<-lapply(D.df,is.null)
D.df<-D.df[null_check!=T]

l_check<-lapply(D.df,ncol)
print(D.df[l_check!=6])
D.df<-D.df[l_check==6]

D.df2<-do.call(rbind, D.df)
tail(D.df2)
write.csv(D.df2,"niche_shift_work/analogue_disagg_21112018/Figures/summary_loss.csv",row.names=F)
test<-read.csv("niche_shift_work/analogue_disagg_21112018/Figures/summary_loss.csv")

x11()
boxplot(log(test$thrown_native+1)~test$region)
